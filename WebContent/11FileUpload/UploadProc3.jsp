...
<%
	//한글깨짐처리(post로 폼값전송시 깨짐부분처리)
	request.setCharacterEncoding("UTF-8");
	

	//파일업로드를위한 MultipartRequest객체의 파라미터 준비
	//1.파일이 실제 업로드될 서버의 물리적경로 가져오기
	/*
		운영체제별로 서버의 물리적 경로는 달라질수 있으므로 파일이
		업로드 되는 정확한 위치를 알기 위해서 반드시 필요한 정보임.
	*/
	...
	/*
		만약 파일을 여러개 업로드 한다면 각각의 용량을 
		합친 용량이 최대용량이 된다.
		예) 500Kb * 2 = 1000Kb
	*/
	int maxPostSize = 1024*5000; //1kb*5000;5Mb
	...
	/*
		...
		예) apple.png, apple1.png, apple3.png 의 형태로
		이름을 자동 부여한다.
	*/
	FileRenamePolicy policy = new DefaultFileRenamePolicy();
	
	//전송된 폼값을 저장하기위한 변수 생성
	MultipartRequest mr = null;
	String name = null;
	String title = null;
	StringBuffer inter = new StringBuffer();//관심사항(체크박스)
	...
	변경하는것이 좋다.
		*/
		...
		//파일명을 변경하기위해 현재시간을 가져옴
		/*
		아래 서식문자중 대문자 S는 밀리초이다 (1/1000)
		*/
		String nowTime = new SimpleDateFormat("yyyy_MM_dd_H_m_s_S").format(new Date());
		
		//파일의 확장자를 가져옴
		/*
		파일명에 닷(.)이 2개 이상 포함될수 있으므로 lastIndexOf()로 
		마지막에 있는 점을 찾아온다. 해당인덱스를 통해 확장자를 가져온다.
		*/
		int idx = -1;
	...
	inter.append(s+", ");
		...
		/*
		파일업로드시 예외가 발생하면 request영역에 오류메세지를 저장하고, 
		업로드폼 페이지로 포워드한다.
		*/
