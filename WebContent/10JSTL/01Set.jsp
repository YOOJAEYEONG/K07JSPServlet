...
-->
	...
	<h3>set태그로 EL안에서 사용할 변수 설정하기</h3>
	
	<ul>
		<li>1.value속성에 직접 값 대입:
			<c:set var="directVar" value="100"/></li>
		<li>2.value속성에 EL로 값 대입:
			<c:set var="elVar" value="${directVar mod 5 }"/></li>
		<li>3.value속성에 표현식으로 값 대입:
			<c:set var="expVar" value="<%=new Date() %>"/></li>
		<li>4.시작태그와 종료태그 사이에 값 설정:
			<c:set var="betweenVar" >변수값 요렇게 설정</c:set></li>
	</ul>
	...
	<h3>EL로 출력</h3>
	<!--  
	EL로 영역의 속성을 읽어올때는 XXXScope를 생략하면 가장 좁은 page영역을 읽어오게 된다. 
	즉아래는 모두 page영역을 읽어오게 된다.
	-->
	<ul>
	...
	<h3>인자생성자로 설정</h3>
	<!--  
	인자생성자를통해 DTO객체를 생성한 후 request영역에 속성을 저장한다.
	-->
	<c:set var="argsMember" 
		value='<%=new MemberDTO("KIM", "1111","김태희",null) %>' 
		scope="request"/>
	<c:set var="argsMember" 
		value='<%=new MemberDTO("KIM1", "1111","김태희",null) %>' 
		scope="page"/>
	
	
	<h3>JSP코드로 출력</h3>
	<!--  
	영역에 저장될때는 Object타입으로 자동형변환 되므로 사용시 해당자료형으로 변환(다운케스팅)해야함-->
	<ul>
		<li>아이디:
			<%=((MemberDTO)request.getAttribute("argsMember")).getId() %></li>
		<li>이름:
			<%=((MemberDTO)request.getAttribute("argsMember")).getName() %></li>
	</ul>
	
	
	<!--  
	EL로 출력시에는 별도로 형변환이 필요없고, 
	속성명이 중복되지 않는다면 xxxScope 내장객체를 생략할 수도 있다.-->
	<h3>EL로 출력</h3>
	<ul>
	...
	<!--
	set태그의 target, property속성은 자바빈 객체의 속성 혹은 컬렉션의 값을 설정할때 
	사용하는 속성이다.
	target:변수값을 지정하는 속성. 반드시 EL, 표현식만 가능
	property:값을 지정하는 속성. 표현식, EL, 값 모두 사용가능
		-자바빈 인 경우 : 속성명(멤버변수)
		-Map 컬렉션인 경우: 키값
		
	**scope속성은 var속성이 지정된 태그에서만 지정가능함
	**target과 property를 사용해서 자바빈이나 컬렉션의 값을 설정할때는
		var속성을 지정할 수 없다.
	-->
	<h2>target속성과 property속성 사용하기</h2>
	<!--1.변수생성  -->
	<c:set var="defaultMember" value="<%=new MemberDTO() %>" scope="request"/> 
	<!--2.생성된변수에 멤버변수값 지정  -->
	
	<%-- <c:set target='<%= request.getAttribute("defaultMember")%>' property="id" value="Jung2" /> --%>
	...
	<c:set target="${list[0] }" property="name" value="쥬니엘"></c:set>
	...
	<!--
	Map 계열의 컬렉션은 Key값을 통해 Value에 접근한다.
	Key를 통한 접근이 index를통한 접근보다 속도가 빠르다.
	-->
	<h2>Map 컬렉션 이용하기</h2>	
	<%
	Map map = new HashMap();
