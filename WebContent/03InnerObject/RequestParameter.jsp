...
<body>
	...
	폼값을 받을 수 있다. 만약 값이 입력되지 않으면 길이가 0인 String객체를
	반환하고, 파라미터명이 틀린경우 null을 반환한다.*/
	...
	if( !(name==null || name.equals("")) ){
		...
		/*스크립트렛 안에서 JS를 선언언할때는 out.println()을통해서
		아래와 같이 사용할 수 있다. 또는 문자열을 연결하여 사용할 수도 있다.*/
		out.println("<script>");
		out.println("	alert('이름을 입력하세요');");
		//out.println("	history.back();");
		//out.println("</script>");
		/* 위2줄을 주석으로 변경하고 아래처럼 바꿔서 기술할 수도 있다. */
	...
	*/
	...
	여러개의 항목중 체크가 된 항목만 전송한다. 파라미터명이 틀리거나 값이
	전송되지 않으면 null을 반환한다.*/
	...
	if(favo != null){
		for(int i=0; i<favo.length ; i++){
			if(i==favo.length-1){
				...
				문자열의 마지막 부분에 스포,경제,정치, 형태로 저장되는것을 방지*/
				favStr += favo[i];				
			}
			else {
				favStr += favo[i] + ",";
	...
	/* 
	textarea로 통해 입력받은 값은 엔터키(줄바꿈기호\r\n)가 포함되어있다.
	...
	getParameterNames() : 폼값의 이름을 한꺼번에 받을때 사용하는
		request 내장 객체의 메소드로 반환타입은 Enumeration형이다.
