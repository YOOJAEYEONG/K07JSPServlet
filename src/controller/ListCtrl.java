...
public class ListCtrl extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//컨트롤러(서블릿) 및 View(jsp)로 전달할 파라미터를 저장하기위한 맵
		Map param = new HashMap();
		//쿼리스트링의 파라미터를 저장하기위한 문자열
...
>>>>>>> refs/remotes/origin/master
			...
			addQueryString =
				String.format(
						"searchColumn=%s&searchWord=%s&", searchColumn, searchWord);
...
///////////////////////////////////////////////
		...
		//현제페이지 번호에 대한 파라미터 처리. 첫 진입시 무조건 1페이지로 지정.
		int nowPage = (
...
//		List<DataroomDTO> lists = dao.selectList(param);//변경전
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		처리는 doGet()에서 처리할수있도록 모든 요청을 토스한다.
		 */
		doGet(req, resp);
	}
